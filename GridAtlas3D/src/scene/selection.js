...
function selectAsset(id,focus=true){const a=get(id);if(!a)return;state.selected=a.assetId;state.trend=rootAsset(a).type==='transformer'?'p':'voltage';const leftBody=$('.left-body');if(leftBody)leftBody.scrollTop=0;if(state.path)calculatePath();renderDetail();refreshVisuals();if(focus)focusAsset(a.assetId);const sldEl=$('#sld [data-asset="'+(a.parent||a.assetId)+'"]');if(sldEl)sldEl.scrollIntoView({block:'nearest',inline:'nearest'});const tree=$(`.tree-item[data-asset="${a.assetId}"]`);if(tree){let p=tree.parentElement;while(p&&p!==$('#asset-tree')){if(p.tagName==='DETAILS')p.open=true;p=p.parentElement;}tree.scrollIntoView({block:'nearest'});}if(innerWidth<=900){document.body.classList.add('mobile-left');document.body.classList.remove('mobile-right');}}
