...
function renderEvents(){$('#alarm-ack').classList.toggle('hidden',!state.alarm||state.alarmAck);$('#event-count').textContent=events.length;$('[data-bottom-tab="events"]').setAttribute('aria-label','OLAYLAR '+events.length);$('#events').innerHTML=events.map(e=>`<button class="event-row ${e.level}" data-event="${e.id}" title="İlgili ekipmanı seç"><time class="mono">${time(e.time)}</time><span class="severity">${e.level==='alarm'?'! ALARM':e.level==='warning'?'! WARNING':'● EVENT'}</span><span>${esc(e.message)}</span><span class="muted mono">${get(e.assetId).tag}</span></button>`).join('');}

export { events, eventSequence, replay, addEvent, renderEvents };
