(function(A){
  ...
  A.Utils = {
    ...
    async fetchJson(url,{signal,timeout=20000,cacheKey,ttl=0}={}){
      ...
      const ctrl=new AbortController(),timer=setTimeout(()=>ctrl.abort('timeout'),timeout);if(signal){if(signal.aborted)ctrl.abort(signal.reason);else signal.addEventListener('abort',()=>ctrl.abort(signal.reason),{once:true});}
