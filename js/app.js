(function(A){
  ...
  class Application{
    ...
    bindUI(){
      ...
      document.getElementById('clearSmokeLayer').addEventListener('click',()=>{document.querySelectorAll('.smokeLayer').forEach(x=>x.checked=false);this.state.smokeVariable=null;this.state.smokeData=[];this.map.clearSmoke();this.ui.renderExportSummary(this.state);});
    ...
    togglePlay(){const b=document.getElementById('playBtn');if(this.playTimer){clearInterval(this.playTimer);this.playTimer=null;b.textContent='▶';return;}b.textContent='⏸';this._playbackApiThrottle=0;this.playTimer=setInterval(()=>{const s=document.getElementById('timeSlider');const step=this.state.mtgEnabled?C.timeline.mtgPlayStepMinutes/60:C.timeline.playStepHours;let v=Number(s.value)+step;if(v>Number(s.max))v=Number(s.min);s.value=String(v);const now=Date.now();const slowReload=now-this._playbackApiThrottle>C.timeline.playIntervalMs*4;this.setTimeOffset(v,slowReload);if(slowReload)this._playbackApiThrottle=now;},C.timeline.playIntervalMs);}
    ...
    async loadSmokeGrid(){
      ...
      if(!pts.length){this.state.smokeData=[];this.state.wildfireSummaryData=[];this.map.clearSmoke();this.ui.setNearestNote('Harita Türkiye veri alanının dışında. Harita serbesttir; CAMS duman sorgusu yalnız Türkiye ile kesişen görünümde yapılır.');this.ui.updateEnvironmentalKpis([],this.state.windData);return;}
