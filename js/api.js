(function(A){
  ...
  A.FirmsAdapter={
    mapKey(){return sessionStorage.getItem('firmsMapKey')||'';},useProxy(){return localStorage.getItem('firmsUseProxy')==='1';},source(){return localStorage.getItem('firmsSource')||'VIIRS_NOAA21_NRT';},
    setKey(v){if(v)sessionStorage.setItem('firmsMapKey',v);else sessionStorage.removeItem('firmsMapKey');},setProxy(v){localStorage.setItem('firmsUseProxy',v?'1':'0');},setSource(v){if(C.firmsSources.includes(v))localStorage.setItem('firmsSource',v);},
    async load(signal){const source=this.source(),bbox=U.regionBboxString(),days=2;let url;if(this.useProxy())url=`/api/firms?bbox=${encodeURIComponent(bbox)}&days=${days}&source=${encodeURIComponent(source)}`;else{const key=this.mapKey();if(!key){const e=new Error('FIRMS MAP_KEY gerekli');e.kind='AUTH_REQUIRED';report('firms',{state:'warn',note:'MAP_KEY gerekli'});throw e;}url=`${C.firmsBase}/${encodeURIComponent(key)}/${source}/${bbox}/${days}`;}
