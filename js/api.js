(function(A){
  ...
  A.OpenMeteoAir={
    ...
    async grid(points,variable,targetTime,signal){
      ...
      try{
        const result=await batches(points,45,async(part,bi)=>{
          ...
          const key=`smoke:${C.activeCountryCode}:${variable}:${bi}:${pkey(part)}`;const {data,meta}=await U.fetchJson(`${C.openMeteoAir}?${params}`,{signal,cacheKey:key,ttl:C.cacheTtl.air});
        ...
        report('air',{state:'ok',latency:Math.round(performance.now()-started),count:result.filter(x=>x.value!=null).length,note:`${metaVar.label} · ${A.activeCountry().nameTr} grid · CAMS Europe`});return result;
    ...
    async detail(point,targetTime,signal){
      ...
      const params=new URLSearchParams({latitude:point.lat.toFixed(5),longitude:point.lon.toFixed(5),hourly:vars.join(','),past_hours:'24',forecast_hours:'96',timezone:'GMT',domains:'cams_europe'}),key=`smokedetail:${C.activeCountryCode}:${point.lat.toFixed(4)},${point.lon.toFixed(4)}`;
  ...
  A.OpenMeteoWeather={
    ...
    async detail(point,targetTime,signal){
      ...
      const params=new URLSearchParams({latitude:point.lat.toFixed(5),longitude:point.lon.toFixed(5),hourly:vars.join(','),past_hours:'24',forecast_hours:'96',timezone:'GMT',wind_speed_unit:'kmh'}),key=`weatherdetail:${C.activeCountryCode}:${point.lat.toFixed(4)},${point.lon.toFixed(4)}`;
  ...
  A.FirmsAdapter={
    ...
    async loadSingle(source, bbox, days, signal, key){
      ...
      const {data,meta}=await U.fetchText(url,{signal,cacheKey:`firms:${C.activeCountryCode}:${source}:${bbox}`,ttl:C.cacheTtl.firms});
    ...
    async loadAll(signal){
      ...
      report('firms',{
        ...
        note: `${A.activeCountry().nameTr} · ${successCount}/${sources.length} VIIRS ürünü başarılı · ${deduped.length} benzersiz tespit (${all.length} hamdan)`
      });
      return deduped;
    },
    async load(signal){
      ...
      try{
        const {data,meta}=await U.fetchText(`${C.firmsBase}/${encodeURIComponent(key)}/${source}/${bbox}/${days}`,{signal,cacheKey:`firms:${C.activeCountryCode}:${source}:${bbox}`,ttl:C.cacheTtl.firms});
        const rows=U.parseCsv(data), out=[];
        for(const r of rows){const parsed = parseFirmsRow(r, source, key, bbox); if(parsed) out.push(parsed);}
        report('firms',{state:'ok',latency:Math.round(performance.now()-started),cached:meta.cached,count:out.length,note:`${source} · ${A.activeCountry().nameTr} bbox + MultiPolygon filtresi`});
        return out;
