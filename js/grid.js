  ...
  class GridRepository{
    ...
    analyzeEvents(events,maxKm=25,referenceTime=new Date(),windData=[]){
      ...
      for(const event of events||[]){
        ...
        const age=U.ageHours(event.latestDetectedAt,referenceTime),distanceScore=minDistanceKm==null?0:minDistanceKm<=1?50:minDistanceKm<=3?42:minDistanceKm<=10?28:minDistanceKm<=25?12:0,frpScore=Math.min(20,Math.sqrt(Math.max(0,event.maxFrp||0))*2),ageScore=age<=3?15:age<=6?12:age<=12?8:age<=24?4:1;
        ...
        const nw=U.nearestPoint(event,windData),wind=nw?.point;let downwindAlignment=false,windScore=0,downwindDirection=null,downwindAssets={lines:[],substations:[]};if(wind&&Number.isFinite(wind.direction)){downwindDirection=(wind.direction+180)%360;downwindAssets=this.assetsInSector(event,downwindDirection);if(nearestAssetPoint){const bearing=U.bearingDeg(event,nearestAssetPoint),diff=U.angleDiff(downwindDirection,bearing);downwindAlignment=diff<=35;if(downwindAlignment)windScore=10;else if(diff<=60)windScore=5;}if(!downwindAlignment&&(downwindAssets.lines.length||downwindAssets.substations.length))windScore=Math.max(windScore,4);}
        ...
        out.push({event,memberAnalyses,nearest,minDistanceKm,band,affectedLines:[...lines.values()],affectedSubstations:[...subs.values()],riskScore:score,riskBand:scoreBand,ageHours:age,wind,downwindDirection,downwindAlignment,downwindAssets,nearestAssetKind,nearestAsset});
      }
