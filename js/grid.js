(function(A){
  ...
  class GridRepository{
    constructor(){this.countryCode='TR';this.manifest=null;this.cellSize=.5;this.reset();}
    reset(){this.data=new Map();this.loading=new Map();this.segmentCells=new Map();this.substationCells=new Map();this.segmentSeq=0;this.loadedCore=false;}
    setCountry(countryCode,manifest=null){this.countryCode=countryCode;this.manifest=manifest;this.reset();}
    ...
    assetKey(props,prefix='line'){return props?.assetId||`${prefix}:${props?.osmId||props?.name||JSON.stringify([props?.actualVoltageKv,props?.operator]).slice(0,120)}`;}
    async loadGroup(key,signal){
      if(this.data.has(key))return this.data.get(key);if(this.loading.has(key))return this.loading.get(key);const cfg=C.gridSources[key],country=this.countryCode;if(!cfg)throw new Error(`Bilinmeyen grid grubu: ${key}`);
      const loadingMap=this.loading,p=(async()=>{const t0=performance.now(),cacheKey=`grid:${country}:${key}`;try{let data=A.Cache.get(cacheKey),mode='country cache';if(!data){const r=await fetch(`${cfg.file}?v=${encodeURIComponent(C.appVersion)}`,{signal,cache:'no-store',headers:{Accept:'application/geo+json,application/json'}});if(!r.ok)throw new Error(`HTTP ${r.status}`);data=await r.json();A.Cache.set(cacheKey,data,C.cacheTtl.grid);mode='HTTP GeoJSON';}if(country!==this.countryCode||country!==C.activeCountryCode){const stale=new Error('Eski ülke şebeke yanıtı');stale.kind='ABORTED';throw stale;}this.data.set(key,data);this.index(key,data);A.Events.emit('service',{id:'grid',state:'ok',latency:Math.round(performance.now()-t0),count:(data.features||[]).length,note:`${A.COUNTRIES[country].nameTr} · ${cfg.label} · ${mode} · OSM/ODbL`});return data;}catch(e){if(e.name==='AbortError')e.kind='ABORTED';if(e.kind!=='ABORTED')A.Events.emit('service',{id:'grid',state:'error',note:`${A.COUNTRIES[country].nameTr} · ${cfg.label}: ${e.message}`});throw e;}finally{loadingMap.delete(key);}})();loadingMap.set(key,p);return p;
    }
    async loadCore(signal){if(this.loadedCore)return;await Promise.all([this.loadGroup('400',signal),this.loadGroup('154',signal),this.loadGroup('substations',signal)]);if(signal?.aborted||this.countryCode!==C.activeCountryCode)return;this.loadedCore=true;A.Events.emit('gridCoreReady',this.stats());}
    index(key,data){
      if(key==='substations'){
        for(const f of data.features||[]){const c=f.geometry?.coordinates,props=f.properties||{};if(props.countryCode!==this.countryCode||!c||!Number.isFinite(c[0])||!Number.isFinite(c[1]))continue;const p={lon:c[0],lat:c[1],props,id:this.assetKey(props,'substation')};const ck=this.cellKey(p.lon,p.lat);if(!this.substationCells.has(ck))this.substationCells.set(ck,[]);this.substationCells.get(ck).push(p);}return;
      }
      for(const f of data.features||[]){const g=f.geometry||{},props={...(f.properties||{}),gridClass:f.properties?.gridClass||key,gridGroup:f.properties?.gridClass||key};if(props.countryCode!==this.countryCode)continue;const assetKey=this.assetKey(props,`line-${key}`);const lines=g.type==='LineString'?[g.coordinates]:g.type==='MultiLineString'?g.coordinates:[];for(const line of lines){for(let i=1;i<line.length;i++){const a={lon:Number(line[i-1][0]),lat:Number(line[i-1][1])},b={lon:Number(line[i][0]),lat:Number(line[i][1])};if(!Number.isFinite(a.lon)||!Number.isFinite(a.lat)||!Number.isFinite(b.lon)||!Number.isFinite(b.lat))continue;const seg={sid:++this.segmentSeq,a,b,props,assetKey,gridClass:props.gridClass,gridGroup:props.gridClass};for(const ck of this.cellsForBbox(Math.min(a.lon,b.lon),Math.min(a.lat,b.lat),Math.max(a.lon,b.lon),Math.max(a.lat,b.lat))){if(!this.segmentCells.has(ck))this.segmentCells.set(ck,[]);this.segmentCells.get(ck).push(seg);}}}
      }
    }
    ...
    stats(){const counts={};for(const [k,v] of this.data)counts[k]=v.features?.length||0;return{countryCode:this.countryCode,manifest:this.manifest,counts,segments:this.segmentSeq,substations:[...this.substationCells.values()].reduce((n,a)=>n+a.length,0)};}
  }
