(function(A){
  ...
  class MapManager{
    ...
    renderFires(selectedTime){
      ...
      if(this.zoom()<9){for(const ev of this.fireEventsVisible){const count=ev.count,radius=U.clamp(7+Math.sqrt(count)*2.2+Math.sqrt(Math.max(0,ev.maxFrp))*0.35,8,24),opacity=U.ageOpacity(ev.latestDetectedAt,new Date(end)),m=new HexagonMarker([ev.lat,ev.lon],{pane:'firePane',renderer:this.renderer,radius,color:'#fff',weight:1.2,opacity,fillColor:U.frpColor(ev.maxFrp),fillOpacity:opacity*.92});m.bindTooltip(`<strong>Yangın olayı kümesi</strong><br>${count} FIRMS termal tespiti<br>Maks. FRP: ${U.round(ev.maxFrp,1)} MW<br>Son tespit: ${U.formatLocal(new Date(ev.latestDetectedAt))}<br><small>5 km / 6 saat kümelenmiş; yangın perimetresi değildir.</small>`);if(this.zoom()<7&&count>1)m.bindTooltip(m.getTooltip().getContent()+`<br><strong>● ${count}</strong>`);m.on('click',e=>{L.DomEvent.stopPropagation(e);this.onPointClick?.({lat:ev.lat,lon:ev.lon,fire:ev.representative,fireEvent:ev});});m.addTo(this.fireLayer);}}
      else{const bounds=this.map.getBounds();const inView=this.fireVisible.filter(f=>f.frp==null||f.frp>=this.frpThreshold).filter(f=>bounds.contains([f.lat,f.lon])).sort((a,b)=>Math.abs(b.frp||0)-Math.abs(a.frp||0)).slice(0,5000);for(const f of inView){const radius=U.clamp(4+Math.sqrt(Math.max(0,f.frp||0))*1.05,4,17),opacity=U.ageOpacity(f.detectedAt,new Date(end)),m=new HexagonMarker([f.lat,f.lon],{pane:'firePane',renderer:this.renderer,radius,color:'#fff',weight:1,opacity,fillColor:U.frpColor(f.frp),fillOpacity:opacity*.9});m.bindTooltip(`<strong>NASA FIRMS termal tespiti</strong><br>${U.escapeHtml(f.product)}<br>FRP: ${f.frp??'—'} MW<br>${U.formatLocal(new Date(f.detectedAt))}<br><small>Hotspot = yangın perimetresi değildir.</small>`);m.on('click',e=>{L.DomEvent.stopPropagation(e);this.onPointClick?.({lat:f.lat,lon:f.lon,fire:f});});m.addTo(this.fireLayer);}}
    ...
    setDownwindCorridors(analyses,show=true){
      ...
      if(count){
        ...
        this.makeLegend('downwind','Rüzgâr Bazlı İzleme Koridoru',`<div class="legendLine"><i style="background:#7be6ff"></i><span>Koridordaki şebeke varlığı</span></div><div class="legendLine"><span class="substationSquare" style="display:inline-block;width:10px;height:10px;background:#0a2531;border:2px solid #7be6ff"></span><span>Koridordaki trafo merkezi</span></div><div class="sourceNote">Yangın olayından maksimum ${C.downwindMaxDistanceKm} km, ±${C.downwind.halfAngleDeg}° sektör. Rüzgâr alanını operasyonel tarama için kullanır; gerçek duman yörüngesi değildir.</div>`);
      }
    }
    ...
    riskSubstationIcon(level,color){const size={critical:14,high:12,medium:10,watch:8,low:8}[level]||10;return L.divIcon({className:'substationIconWrap',html:`<span class="substationSquare substation-risk-${level==='watch'?'low':level}" style="width:${size}px;height:${size}px;background:${color};border-color:#fff"></span>`,iconSize:[size,size],iconAnchor:[size/2,size/2]});}
    sectorSubstationIcon(){return L.divIcon({className:'substationIconWrap',html:`<span class="substationSquare" style="width:10px;height:10px;background:#0a2531;border:2px solid #7be6ff"></span>`,iconSize:[10,10],iconAnchor:[5,5]});}
