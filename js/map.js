(function(A){
  ...
  class MapManager{
    ...
    async setGridGroup(key,data,show){
      ...
      let renderData=data;
      if(key==='substations'&&this.zoom()<7&&(data.features?.length||0)>1500){const stride=Math.ceil(data.features.length/1500);renderData={...data,features:data.features.filter((_,i)=>i%stride===0)};}
      let layer;
    ...
    gridTooltip(p,isSub){const actual=U.formatVoltage(p.actualVoltageKv)||'Bilinmiyor',title=isSub?'Trafo Merkezi':'İletim Hattı',rows=[`<strong>${title}</strong>`,U.escapeHtml(p.name||(isSub?'Adsız trafo merkezi':'Adsız OSM hattı'))];if(!isSub)rows.push(`Hat sınıfı: ${U.escapeHtml(p.displayClass||`${p.gridClass} kV sınıfı`)}`);rows.push(`Gerçek OSM gerilimi: ${U.escapeHtml(actual)}`);if(p.operator)rows.push(U.escapeHtml(p.operator));rows.push('<small>OpenStreetMap / ODbL 1.0</small>');return rows.join('<br>');}
